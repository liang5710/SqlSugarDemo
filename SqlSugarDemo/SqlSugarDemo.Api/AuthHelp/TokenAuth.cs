...
namespace ExampleDemo.Core.AuthHelp
{
    ...
    public class TokenAuth
    {
        ...
        public Task Invoke(HttpContext httpContext)
        {
            ...
            //检查是否包含‘Authorization’请求头，如果不包含返回context 进行下一个中间件，用于访问不需要认证的API
            ...
            try
            {
                string jwtStr = tokenStr.ToString().Substring("liang".Length).Trim();
                //验证缓存中是否存在该jwt字符串
                if (!DemoMemoryCache.Exists(jwtStr))
                {
                    return httpContext.Response.WriteAsync("非法请求");
                }
                TokenModel tm = (TokenModel)DemoMemoryCache.Get(jwtStr);
